...
if ($real_hash != $hash)
{
    ...
    $result = mysqli_query($link, "INSERT INTO  " . PLAYERS_DB . " (`name` , `nick` , `password` , `ip`, `email`, `verify`, `active`, `coins`, `address` ) VALUES ('" . $name . "' ,  '" . $nick . "' ,  '" . $password . "' ,  '" . $mIP . "',  '" . $email . "',  '" . $random_hash . "',  '" . $emailVerification . "',  '" . $coins . "',  '" . $address . "') ") or die(mysqli_error($link));
    ...
    if ($result) {
        if ($emailVerification == 0) {
            ...
            $body    = 'Hi ' . $name . '<br/>Your Account has been create, to sign in please verify your email.<br/> <br/> Please Click On This link or paste in your browser: <a href="' . $burl . 'Activation.php?code=' . $random_hash . '">' . $burl . 'Activation.php?code=' . $random_hash . '</a> to activate  your account.';
            $headers = "From:" . $from . "\r\n";
